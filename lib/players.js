...
import { resultKey } from '../public/assets/js/league.js';
...
/**
 * Ce que rapporte un pronostic. Les trois paliers s'excluent : le score exact
 * vaut quinze packs, pas quinze plus trois plus un.
 */
export const PREDICTION_REWARDS = { exact: 15, result: 3, played: 1 };
...
/** Score maximal accepté : au-delà, c'est une faute de frappe. */
export const SCORE_MAX = 30;

/**
 * Départage un pronostic face au score réel.
 * @returns {'exact'|'result'|'played'}
 */
export function predictionOutcome(prediction, match) {
  const pourNous = Number(prediction?.home);
  const contreNous = Number(prediction?.away);
  if (pourNous === match.homeScore && contreNous === match.awayScore) return 'exact';
  if (resultKey(pourNous, contreNous) === resultKey(match.homeScore, match.awayScore)) return 'result';
  return 'played';
}

export function rewardFor(outcome) {
  return PREDICTION_REWARDS[outcome] || 0;
}

/**
 * @returns {{ok: true, matchId: string, home: number, away: number} | {ok: false, error: string}}
 */
export function validatePrediction(input) {
  const matchId = String(input?.matchId ?? '').trim().slice(0, 80);
  const home = Number(input?.home);
  const away = Number(input?.away);

  if (!matchId) return { ok: false, error: 'Match inconnu.' };
  const correct = (valeur) => Number.isInteger(valeur) && valeur >= 0 && valeur <= SCORE_MAX;
  if (!correct(home) || !correct(away)) {
    return { ok: false, error: `Indiquez deux scores entiers, entre 0 et ${SCORE_MAX}.` };
  }
  return { ok: true, matchId, home, away };
}
