...
/**
 * Le décompte des réponses, tous paris confondus.
 *
 * Une seule requête plutôt qu'une par pari : la page en affiche une dizaine, et
 * le regroupement coûte moins cher côté base que dix allers-retours. À qui la
 * montrer, c'est l'affaire de l'API — voir `/api/club/predictions`.
 *
 * @returns {Promise<Object<string, Object<string, number>>>} pari → réponse → nombre
 */
export async function tallyWagers(db) {
  return withSchema(db, async () => {
    const { results } = await db
      .prepare('SELECT bet_id, answer, COUNT(*) AS count FROM wagers GROUP BY bet_id, answer')
      .bind().all();
    const parPari = {};
    for (const ligne of results || []) {
      (parPari[ligne.bet_id] ||= {})[String(ligne.answer)] = Number(ligne.count) || 0;
    }
    return parPari;
  });
}
