...
const SCHEMA = [
  ...
  /*
   * Pronostics. La clé primaire composite est ce qui rend le pronostic
   * modifiable sans jamais se dédoubler : un parieur n'a qu'une seule
   * prévision par match, corrigeable tant qu'elle n'est pas réglée.
   */
...
export async function listPredictions(db, userId) {
  return withSchema(db, async () => {
    const { results } = await db
      .prepare(`SELECT id, match_id, home, away, created_at, settled_at, outcome, awarded
...
/**
 * Enregistre ou corrige un pronostic.
 *
 * La clause `WHERE predictions.settled_at IS NULL` du `DO UPDATE` interdit de
 * changer d'avis une fois le match réglé : sans elle, on pourrait rejouer le
 * bon score après coup et réclamer les quinze packs.
 *
 * @returns {Promise<boolean>} `false` si le pronostic était déjà réglé.
 */
export async function savePrediction(db, userId, matchId, home, away) {
  return withSchema(db, async () => {
    const resultat = await db
      .prepare(`INSERT INTO predictions (user_id, match_id, home, away, created_at)
                VALUES (?, ?, ?, ?, ?)
      ...
      .bind(userId, matchId, home, away, new Date().toISOString()).run();
    return Number(resultat?.meta?.changes ?? 0) > 0;
  });
}

/**
 * Règle un pronostic et verse les packs gagnés.
 *
 * Le versement suit la mise à jour conditionnelle : si la ligne était déjà
 * réglée, rien n'est versé. En cas d'interruption entre les deux ordres, le
 * pronostic reste réglé sans crédit — on préfère cette perte-là au double
 * versement, qui serait invisible et cumulatif.
 *
 * @returns {Promise<boolean>} `true` si c'est bien cet appel qui l'a réglé.
 */
export async function settlePrediction(db, id, userId, outcome, packs) {
  return withSchema(db, async () => {
    const resultat = await db
      .prepare(`UPDATE predictions SET settled_at = ?, outcome = ?, awarded = ?
...
/** Le tableau des meilleurs pronostiqueurs : les packs gagnés font foi. */
export async function topPredictors(db, limit = 10) {
  return withSchema(db, async () => {
    const { results } = await db
      .prepare(`SELECT u.username AS username,
                       SUM(p.awarded) AS packs,
                       COUNT(p.id) AS total,
                       SUM(CASE WHEN p.outcome = 'exact' THEN 1 ELSE 0 END) AS exacts
                FROM predictions p
