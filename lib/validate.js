...
function sanitizeTeam(input, index, used) {
  ...
  return {
    ...
    // false = adversaire rencontre hors championnat : absent du classement.
    inLeague: input?.inLeague !== false
  };
}
...
function sanitizeMatch(input, index, used) {
  ...
  return {
    ...
    // Un match de coupe nourrit la forme du moment sans peser au classement.
...
function sanitizeChampionship(input = {}) {
  ...
  const matches = arr(input.matches, LIMITS.matches, (match, index) => sanitizeMatch(match, index, rencontres));

  /*
   * Un club supprime alors qu'un match le cite encore laissait une reference
   * orpheline : le site affichait « — » a sa place, et le classement l'ignorait
   * en silence. On le recree sous un nom devine, visible dans l'administration,
   * ou il n'y a plus qu'a le renommer. Supprimer ses matchs le fait disparaitre
   * pour de bon.
   */
  for (const match of matches) {
    for (const id of [match.homeId, match.awayId]) {
      ...
      equipes.add(id);
      // Recree hors poule s'il n'apparait que dans des matchs sans classement.
      const inLeague = matches.some((autre) =>
        (autre.homeId === id || autre.awayId === id) && autre.ranked !== false);
      teams.push({
        id, name: humanize(id), short: humanize(id).slice(0, 16),
        logo: '', penalty: 0, inLeague
      });
    }
  }
