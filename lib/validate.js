...
/** Sert au seul formulaire de contact : le club, lui, ne publie plus d'adresse. */
...
/** Un barème : ce que rapporte chaque palier, en packs. */
function sanitizeRewards(input = {}, defauts) {
  return {
    exact: int(input.exact, 0, 999, defauts.exact),
    result: int(input.result, 0, 999, defauts.result),
    played: int(input.played, 0, 999, defauts.played)
  };
}

/**
 * Un pari saisi par le bureau.
 *
 * Le point sensible est `answers` : c'est la bonne réponse, et elle vaut des
 * packs. On ne garde que des options qui existent vraiment — une réponse
 * fantôme rendrait le pari impossible à gagner, sans que rien ne le signale.
 */
function sanitizeBet(input, index, used, matchIds) {
  if (!input || typeof input !== 'object') return null;
  const question = str(input.question, 200);
  if (!question) return null;

  const type = oneOf(input.type, ['score', 'result', 'choice'], 'choice');
  const surUnMatch = type === 'score' || type === 'result';
  const matchId = matchIds.has(slug(input.matchId, '')) ? slug(input.matchId, '') : '';

  /*
   * Un pari 1/N/2 ou de score sans match ne peut ni fermer ni se régler : il
   * redevient une question ouverte, que le bureau tranchera lui-même.
   */
  const nature = surUnMatch && !matchId ? 'choice' : type;

  const vues = new Set();
  const options = nature === 'choice'
    ? arr(input.options, LIMITS.betOptions, (option, rang) => {
        const label = str(option?.label, 90);
        if (!label) return null;
        return { id: uniqueId(option?.id || label, `option-${rang + 1}`, vues), label };
      })
    : [];

  /*
   * Un pari sur un match tire sa bonne réponse du score, pas d'une saisie : on
   * n'en garde aucune. Ailleurs, seules les options réellement proposées sont
   * acceptées — une réponse fantôme rendrait le pari impossible à gagner.
   */
  const connues = new Set(options.map((option) => option.id));
  const retenues = new Set();
  const answers = nature !== 'choice' ? [] : arr(input.answers, LIMITS.betOptions, (id) => {
    const reponse = str(id, 60);
    if (!connues.has(reponse) || retenues.has(reponse)) return null;
    retenues.add(reponse);
    return reponse;
  });

  return {
    id: uniqueId(input.id || question, `pari-${index + 1}`, used),
    type: nature,
    question,
    note: str(input.note, LIMITS.medium),
    matchId,
    options,
    // Désigner la bonne réponse ferme le pari : voir `isOpen` dans bets.js.
    answers,
    closesAt: isoDate(input.closesAt),
    closed: input.closed === true,
    rewards: sanitizeRewards(input.rewards, { exact: 5, result: 0, played: 1 })
  };
}

function sanitizeBets(input = {}, matchIds = new Set()) {
  const used = new Set();
  return {
    title: str(input.title, 90) || 'Les paris',
    subtitle: str(input.subtitle, LIMITS.medium),
    intro: str(input.intro, LIMITS.body),
    // Coupé, plus aucun pronostic ne s'ouvre tout seul sur les matchs.
    autoMatch: input.autoMatch !== false,
    matchRewards: sanitizeRewards(input.matchRewards, { exact: 15, result: 3, played: 1 }),
    items: arr(input.items, LIMITS.bets, (bet, index) => sanitizeBet(bet, index, used, matchIds))
  };
}
...
function sanitizeSquad(input = {}) {
  ...
  const players = arr(input.players, LIMITS.players, (player, index) => {
      ...
      return {
        ...
        firstName,
        lastName,
        // Surnom de vestiaire : affiche entre guillemets sous le nom.
        nickname: str(player?.nickname, 40),
...
export function sanitizeContent(input) {
  ...
  // Les paris désignent des matchs : on leur passe la liste réellement retenue,
  // pour qu'un pari ne pointe jamais vers un match qui vient d'être supprimé.
  const championship = sanitizeChampionship(input.championship);
  const matchIds = new Set(championship.matches.map((match) => match.id));
  ...
  return {
    ...
    championship,
    bets: sanitizeBets(input.bets, matchIds),
    squad,
